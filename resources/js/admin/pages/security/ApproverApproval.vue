...
const requestSummary = computed(() => [
  ...
  {
    label: "Batas Waktu",
    value: props.changeRequest.expires_at || "Tidak tersedia",
    icon: "M12 6v6l4 2m5-2a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z",
  },
  {
...
const oldData = computed(() => [
  { label: "Nama", value: props.changeRequest.old_name || "-" },
  { label: "Jabatan", value: props.changeRequest.old_position || "-" },
  { label: "Nomor WhatsApp", value: props.changeRequest.old_whatsapp_number || "-" },
]);

const newData = computed(() => [
  { label: "Nama", value: props.changeRequest.new_name || "-" },
  { label: "Jabatan", value: props.changeRequest.new_position || "-" },
  { label: "Nomor WhatsApp", value: props.changeRequest.new_whatsapp_number || "-" },
]);

const approve = () => {
  approveForm.post(
    `/admin/security/approver-approval/${props.changeRequest.token}/approve`,
    {
      preserveScroll: true,
    }
  );
...
const reject = () => {
  rejectForm.post(
    `/admin/security/approver-approval/${props.changeRequest.token}/reject`,
    {
      preserveScroll: true,
    }
  );
...
<template>
  <Head title="Approval Perubahan Approver" />
      ...
      >
          ...
          >
            <div class="max-w-3xl">
              ...
              >
                ...
                <span class="block text-red-300">Nomor Approver</span>
              </h1>

              <p
                class="mt-4 max-w-2xl text-justify text-sm leading-8 text-slate-300 sm:text-base"
              >
                Tinjau data lama dan data baru sebelum menyetujui atau menolak perubahan
                nomor Ketua HMPS sebagai approver keamanan website.
              </p>
            </div>
        ...
        <div class="space-y-6 px-5 py-6 sm:px-7 lg:px-8 lg:py-8">
          ...
          >
            ...
            <div
              class="grid gap-4 p-5 sm:p-6 lg:grid-cols-[1fr_auto_1fr] lg:items-stretch"
            >
              <!-- Old data -->
              <div class="rounded-[1.5rem] border border-white/10 bg-white/[0.045] p-5">
                <div class="mb-5 flex items-center gap-3">
                  <span
                    class="flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-400/10 text-slate-300 ring-1 ring-white/10"
                  >
                    <svg
                      class="h-5 w-5"
                      fill="none"
                  ...
                  <div>
                    ...
                    <p class="mt-0.5 text-sm font-semibold text-slate-500">
                      Nomor yang sedang/terakhir aktif
                    </p>
                  </div>
                </div>
              ...
              <div class="hidden items-center justify-center lg:flex">
                <div
                  class="flex h-14 w-14 items-center justify-center rounded-2xl border border-red-300/20 bg-red-500/10 text-red-200 shadow-[0_0_40px_rgba(239,68,68,0.12)]"
                >
                  <svg
                    class="h-6 w-6"
                    fill="none"
              ...
              <!-- New data -->
              <div
                class="rounded-[1.5rem] border border-emerald-400/20 bg-emerald-500/10 p-5"
              >
                <div class="mb-5 flex items-center gap-3">
                  <span
                    class="flex h-11 w-11 items-center justify-center rounded-2xl bg-emerald-400/12 text-emerald-200 ring-1 ring-emerald-300/20"
                  >
                    <svg
                      class="h-5 w-5"
                      fill="none"
                  ...
                  <div>
                    <p
                      class="text-xs font-black uppercase tracking-[0.12em] text-emerald-200"
                    >
                      Data Baru
                    </p>
                    <p class="mt-0.5 text-sm font-semibold text-emerald-100/70">
                ...
                <div class="space-y-3">
                  <div
                    ...
                    :key="item.label"
                    class="rounded-2xl border border-emerald-300/15 bg-slate-950/18 px-4 py-3"
                  >
                    <p
                      class="text-[0.68rem] font-black uppercase tracking-[0.1em] text-emerald-200/80"
                    >
                      {{ item.label }}
                    </p>
            ...
            <div class="border-t border-white/10 p-5 sm:p-6">
              <div class="rounded-[1.4rem] border border-white/10 bg-white/[0.045] p-5">
                <div class="mb-3 flex items-center gap-3">
                  <span
                    class="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/[0.06] text-red-200 ring-1 ring-white/10"
                  >
                    <svg
                      class="h-5 w-5"
                      fill="none"
                    ...
                    >
                      <path
                        ...
                        stroke-linejoin="round"
                        stroke-width="2.25"
                        d="M8 10h8M8 14h5m8-2a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"
                      />
                    </svg>
                  </span>
                  <div>
                    ...
                    <p class="text-xs font-semibold text-slate-500">
                      Keterangan dari pengajuan perubahan approver
                    </p>
                  </div>
                </div>

                <p class="text-justify text-sm leading-8 text-slate-300">
                  {{ changeRequest.request_reason || "-" }}
                </p>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="relative z-10">
                ...
                <p class="mt-2 text-justify text-sm leading-7 text-emerald-100/78">
                  Jika disetujui, nomor baru langsung menjadi approver aktif untuk proses
                  keamanan website.
                </p>
                ...
                <input
                  v-model="approveForm.processed_by_name"
                  type="text"
                ...
                >
                  ...
                  {{
                    approveForm.processing
                      ? "Memproses..."
                      : "Setujui dan Aktifkan Nomor Baru"
                  }}
                </button>
              </div>
            </form>
            ...
            >
              ...
              <div class="relative z-10">
                ...
                <p class="mt-2 text-justify text-sm leading-7 text-red-100/78">
                  Jika ditolak, nomor approver lama tetap aktif dan perubahan tidak
                  diterapkan ke sistem.
                </p>
                ...
                <input
                  v-model="rejectForm.processed_by_name"
                  type="text"
                ...
                >
                  ...
                  {{ rejectForm.processing ? "Memproses..." : "Tolak Perubahan" }}
                </button>
              </div>
            </form>
          </div>
